...
export const register = (
  ...
  if (username.length <= 5)
    throw AuthError.ValidationError(`Login must be longer than 5 characters`)
  if (password.length <= 6)
    throw AuthError.ValidationError(`Password must be longer than 6 characters`)
  if (userList.some((user) => user.username === username))
    throw AuthError.ValidationError(`Username: "${username}" is already registered`)
